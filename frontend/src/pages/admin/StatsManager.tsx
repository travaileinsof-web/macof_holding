...
const PAGE_SECTIONS: Record<string, { page: string; slug: string; label: string; description: string }[]> = {
  'Page d\'Accueil': [
    { page: 'home', slug: 'stats', label: 'Chiffres Clés (accueil)', description: 'Statistiques affichées dans la section "Chiffres Clés" de la page d\'accueil.' },
  ],
};
...
export default function StatsManager() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (homeRes.data.success && homeRes.data.data?.stats) {
        try {
          setStats(JSON.parse(homeRes.data.data.stats));
        } catch {
          setStats([
      ...
      if (filialesRes.data.success) {
        const raw: any[] = Array.isArray(filialesRes.data.data) ? filialesRes.data.data : (filialesRes.data.data?.items || []);
        setFiliales(raw.map((f) => ({
          id: f.id,
          nom: f.nom,
          slug: f.slug || '',
  ...
  const handleSaveFilialeStats = async (filiale: FilialeStats) => {
    setSaving(true);
    try {
      // Save stat fields via pages API (slug = filiale.slug)
      await api.post('/api/v1/admin/pages/bulk', {
          ...
          <div className="bg-[#1e293b] border border-slate-700 rounded-lg p-6">
            <div className="flex items-center justify-between mb-2">
              <div>
                ...
                <p className="text-slate-400 text-sm mt-1">
                  Ces statistiques s'affichent dans la section blanche de la page d'accueil (Année de création, Filiales, Projets, Collaborateurs, etc.)
                </p>
              </div>
            </div>
            ...
            <div className="mt-6 space-y-3">
              {stats.map((stat, i) => (
                <StatRow key={i + '-' + stat.label} stat={stat} index={i} onChange={handleStatChange} onDelete={handleDeleteStat} />
              ))}
            </div>
      ...
      {activeTab === 'filiales' && (
        <div className="space-y-3">
          <p className="text-slate-400 text-sm">
            Modifiez les deux statistiques affichées sur chaque page de filiale (ex: "3 — Segments Stratégiques", "SEBA — Marque Phare").
          </p>
          {filiales.map((f) => (
            <div key={f.id} className="bg-[#1e293b] border border-slate-700 rounded-lg overflow-hidden">
              ...
              {expandedFiliale === f.id && (
                <div className="px-5 pb-5 border-t border-slate-700 pt-5 space-y-4">
                  <p className="text-slate-400 text-sm">Ces chiffres s'affichent en bas de la section "Vision" de la page <strong className="text-slate-300">{f.nom}</strong>.</p>
                  ...
                  <div className="flex justify-end">
                    ...
                    >
                      ...
                      {saving ? 'Sauvegarde...' : `Sauvegarder ${f.nom}`}
                    </button>
                  </div>
                </div>
              )}
