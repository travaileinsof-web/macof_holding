...
export default function FilialesManager() {
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      if (res.data.success) setFiliales(res.data.data || []);
    } catch (err) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editId) {
        await api.put(`/api/v1/admin/filiales/${editId}`, formData);
      } else {
        await api.post('/api/v1/admin/filiales', formData);
      }
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4">
          <div className="bg-[#1e293b] border border-slate-700 rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1">Details (un par ligne)</label>
                <textarea
                  value={form.details}
            ...
            <div className="flex justify-end gap-3 px-6 py-4 border-t border-slate-700">
              ...
              >
                ...
                {saving ? 'Sauvegarde...' : editId ? 'Modifier' : 'Enregistrer'}
              </button>
            </div>
          </div>
        </div>
