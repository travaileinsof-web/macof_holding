...
export const statutFilialeEnum = pgEnum('statut_filiale', [
  'actif',
  'inactif',
]);
...
export const civiliteEnum = pgEnum('civilite', [
  'monsieur',
  'madame',
]);
...
export const administrateurs = pgTable('administrateurs', {
  ...
  filiale_attribuee: integer('filiale_attribuee'),
  archived: boolean('archived').default(false).notNull(),
  created_at: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
  updated_at: timestamp('updated_at', { withTimezone: true }).defaultNow().notNull(),
});
...
export const demandes_contact = pgTable('demandes_contact', {
  id: serial('id').primaryKey(),
  reference: varchar('reference', { length: 50 }).notNull().unique(),
  filiale: integer('filiale').references(() => filiales.id),
  type_demande: typeDemandeEnum('type_demande').default('information').notNull(),
...
export const catalogues = pgTable('catalogues', {
  ...
  filiale: integer('filiale').references(() => filiales.id),
  type_document: typeDocumentEnum('type_document').default('catalogue').notNull(),
  ...
  index('idx_catalogues_filiale').on(self.filiale),
]);
...
export const galerie = pgTable('galerie', {
  ...
  index('idx_galerie_filiale').on(self.filiale),
]);

// ─── Chatbot logs ───────────────────────────────────────────────────────────

export const chatbot_logs = pgTable('chatbot_logs', {
  id: serial('id').primaryKey(),
  ...
  statut_resolution: statutResolutionEnum('statut_resolution').default('en_attente').notNull(),
  created_at: timestamp('created_at', { withTimezone: true }).defaultNow().notNull(),
}, (self) => [
  index('idx_chatbot_session').on(self.session_id),
]);

// ─── Page contents (CMS) ────────────────────────────────────────────────────

export const page_contents = pgTable('page_contents', {
  id: serial('id').primaryKey(),
